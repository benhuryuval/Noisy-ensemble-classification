...
from auxiliaryFunctions import white_noise, write_csv_line, calc_accuracy, calc_constraint, load_dataset
from AdaBoostModel_xval import AdaBoostModel
...
simIterNum = 250  # number of iterations for each SNR
...
for numBaseClassifiers in numBaseClassifiersList:
    ...
    for database_idx, database in enumerate(databaseList):
        ...
        # load database
        X, y = load_dataset(database, database_path)
        ...
        # split database using k-fold cross-validation
        kf = KFold(n_splits=5, random_state=None, shuffle=False)
        fold_idx = 1
        for train_index, test_index in kf.split(X):
            print("TRAIN:", train_index, "\nTEST:", test_index)
            train_X, test_X = X[train_index], X[test_index]
            train_y, test_y = binary_encoded_y[train_index], binary_encoded_y[test_index]

            # train real adaboost model and calculate confidence levels for train and test sets
            model = AdaBoostModel(train_X, train_y, numBaseClassifiers)
            confidence_levels_train = model.calc_confidence_levels(train_X, confidence_eps)
            confidence_levels_test = model.calc_confidence_levels(test_X, confidence_eps)
            ...
            for powerLimit in powerLimitList:
                    ...
                    pathToCsvFile = resultsFolderPath + database + "_T" + str(numBaseClassifiers) + "_P" + str(powerLimit) + "_fold" + str(fold_idx) + "_" + datetime.datetime.now().strftime("%d_%m_%Y-%H_%M_%S") + ".csv"
                    ...
                    for snr in snr_array:
                        ...
                        curr_snr = np.mean(
                            np.linalg.norm(confidence_levels_train, ord=2, axis=0) ** 2) / sigma_profile.sum()
                        ...
                        if optimization_flag:
                            ...
                            opt_alpha_unitb, opt_beta_unitb = np.ones([numBaseClassifiers, 1]), np.ones(
                        ...
                        if optimization_flag:
                            ...
                            opt_alpha_unifb, opt_beta_unifb = np.ones([numBaseClassifiers, 1]), np.ones(
                                [numBaseClassifiers, 1])
                        ...
                        if optimization_flag:
                            ...
                            opt_alpha_ab_j, opt_beta_ab_j = np.ones([numBaseClassifiers, 1]), np.ones(
                                [numBaseClassifiers, 1])
                        # - * - * - * - * - * - * Training end * - * - * - * - * - * - * -
                        ...
                        for iteration in range(simIterNum):
                            ...
                            noise_matrix = white_noise(sigma, numBaseClassifiers, len(test_y))
                            ...
                            predictTest_noiseless = model.classifier.predict(test_X)  # predicting on non-noisy data as reference
                            # - * - * - * - * - * - * Inference end * - * - * - * - * - * - * -

                            # - * - * - * - * - * - * Evaluation * - * - * - * - * - * - * -
                            # get confusion matrix between noisy inference and true class (for error probability)
                            CM_trivial += sk.metrics.confusion_matrix(test_y, predictTest_trivial[0])
                            CM_unitb += sk.metrics.confusion_matrix(test_y, predictTest_unitb[0])
                            CM_unifb += sk.metrics.confusion_matrix(test_y, predictTest_unifb[0])
                            CM_ab += sk.metrics.confusion_matrix(test_y, predictTest_ab[0])
                            CM_ab_j += sk.metrics.confusion_matrix(test_y, predictTest_ab_j[0])
                    ...
                    print("Completed simulating " + database + "with T=" + str(numBaseClassifiers) + ", G=" + str(powerLimit))
                    ...
                    plt.savefig(resultsFolderPath + 'perrmis_ab_' + database + '_T' + str(numBaseClassifiers) + '_G' + str(int(powerLimit)) + "_fold" + str(fold_idx))
